...
### 명령어 출력(stdout) 제한
- 터미널 명령 결과가 길 경우 `| head -50` 등으로 잘라서 컨텍스트에 포함시킨다.
- 빌드·테스트 결과는 에러 부분만 복붙하거나 `2>&1 | tail -30`으로 마지막 30줄만 전달한다.
- 대형 로그 파일은 Bash `cat` 대신 `Grep`으로 핵심 키워드만 검색해 읽는다.
- `! <command>` 로 긴 출력이 예상될 때는 실행 전에 파이프로 출력 길이를 제한한다.
